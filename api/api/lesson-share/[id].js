...
export default async function handler(req, res) {
  ...
  const targetUrl =
    `${SITE_URL}/disciple.html#${encodeURIComponent(id)}`;

  const shareUrl =
    `${SITE_URL}/api/lesson-share/${encodeURIComponent(id)}`;

  const ogImage =
    `${SITE_URL}/social-preview-lessons.png?v=50`;

  let title = "Lessons from a Disciple";

  let description =
    "Real reflections, lessons, and encouragement from Restored Daily Ministries.";
...
<body>
<p>
<a href="${targetUrl}">Open lesson</a>
</p>
</body>
</html>
`);
